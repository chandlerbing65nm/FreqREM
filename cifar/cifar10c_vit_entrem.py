        ...
        for b in range(n_batches):
            ...
            if prune_mask is not None:
                ...
                if keep_b.any():
                    ...
                    # This call will perform adaptation (EntREM.forward uses enable_grad internally)
                    t0 = time.time()
                    _ = model(x_adapt)
                ...
                # Accumulate EntREM prune categories based on the adaptation pass (only if enabled)
                stats_src = model.module if hasattr(model, 'module') else model
...
def setup_entrem(model):
    ...
    params, param_names = entrem.collect_params(model, ln_quarter=cfg.MODEL.LN_QUARTER)
