...
def evaluate_phase_mix_then_mask_trend(model: torch.nn.Module,
                                        x: torch.Tensor,
                                        y: torch.Tensor,
                                        device: torch.device,
                                        batch_size: int = 50,
                                        ...
                                        example_tag: str = ""):
    """
    Combined pipeline per corruption (x,y):
      For each t in ratios (percent):
        1) Build phase-attenuated counterpart x_phase using alpha.
        2) Mix: x_t = (1 - t) * x + t * x_phase.
        3) Apply ViT patch masking at the SAME t%: keep num_keep = tokens * (1 - t).
        4) Compute error and mean predictive entropy.

    Returns: ratios (list of ints 0..100), errors (list), entropies (list)
    """
    model.eval()

    tokens = 576  # ViT-B/16 @ 384x384 has 24*24 patch tokens
    ...
    with torch.no_grad():
        ...
        for start in tqdm(range(0, N, batch_size), total=(N + batch_size - 1)//batch_size, desc="Batches", leave=False):
            ...
            for r in ratios:
                ...
                # Unmasked forward on xt to get attention
                outputs, attn = model(xt, return_attn=True)  # logits: [B,C], attn: [B, H, 577, 577]
                attn_score = attn.mean(dim=1)[:, 0, 1:]      # [B, 576]

                if r == 0:
                    logits = outputs  # keep all tokens (no masking)
                else:
                    # Masking at the same t%
                    num_keep = int(tokens * (1.0 - t))
                    if num_keep > 0:
                        # Select smallest attention values to keep (mirroring REM behavior)
                        len_keep = torch.topk(attn_score, num_keep, largest=False).indices
                    else:
                        # Keep zero patch tokens; model keeps only CLS token
                        len_keep = attn_score[:, :0]
                    logits = model(xt, len_keep=len_keep, return_attn=False)
            ...
            # Optionally save example mixes (unmasked) for visualization
            if save_mix_examples > 0 and saved_examples < save_mix_examples and figs_dir is not None:
                ...
                for bi in range(to_take):
                    ...
                    for lv in mix_example_levels:
                        tt = float(lv) / 100.0
                        xt1 = (1.0 - tt) * x1 + tt * x1_phase
                        xt1 = xt1.clamp(0.0, 1.0)
                        imgs.append(xt1[0].detach().cpu())
                    ...
                    for ax, im, lab in zip(axes, imgs, labels):
                        ...
                        ax.set_title(lab)
                        ax.axis('off')
                    tag = example_tag or "phase_mix_then_mask"
...
def plot_trend(ratios, errors, entropies, title: str, out_path: str):
    ...
    ax1.set_xlabel('t (%) — mix then mask')
...
def main():
    parser = argparse.ArgumentParser(description='Combined phase-mix then patch-masking trend on CIFAR-10-C for source model (no adaptation).')
    ...
    parser.add_argument('--progression', type=int, nargs=3, metavar=('START','STOP','STEP'), default=[0, 100, 10],
                        help='Progression of t values in percent (both mixing and masking use the same t)')
    ...
    parser.add_argument('--save_mix_examples', type=int, default=0,
                        help='Number of example phase-mixed samples to save per corruption (0 disables)')
    ...
    for ctype in tqdm(args.corruptions, desc="Corruptions"):
        ...
        ratios, errors, entropies = evaluate_phase_mix_then_mask_trend(
        ...
        title = f"{title_map.get(ctype, ctype)}"
